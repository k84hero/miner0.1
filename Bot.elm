    ...
    let
        targetsToUnlock =
            context.readingFromGameClient.targets
                |> List.filter (.textsTopToBottom >> List.any (String.toLower >> String.contains "asteroid" >> String.contains "White Glaze" >> String.contains "Krystallos" >> String.contains "Dark Glitter" >> String.contains "Gellidus" >> String.contains "Glare Crust") >> not)
    in
    targetsToUnlock
        |> List.head
        |> Maybe.map
